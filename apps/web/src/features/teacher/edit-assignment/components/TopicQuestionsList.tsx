      ...
      <div className="flex items-center p-4 hover:bg-gray-50 transition-colors">
        ...
        >
          <div>
            ...
            <div className="text-xs text-gray-500 mt-0.5">{t('teacher.editAssignment.questionsCount', { count: topic._count?.questions || 0 })}</div>
          </div>
        ...
        <div className="border-t border-gray-100 bg-gray-50/50 p-4 space-y-3">
          {isLoading ? (
            ...
            questions.map(q => {
              ...
              return (
                <label key={q.id} className="flex items-start cursor-pointer group bg-white p-3 rounded-xl border border-gray-200 hover:border-indigo-300 transition-colors shadow-sm">
                  <input
                    type="checkbox"
                    className="mt-0.5 mr-3 w-4 h-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500 cursor-pointer"
                    checked={isChecked}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs font-semibold">
                      <span className={`px-2 py-0.5 rounded-md ${
                        ...
                        {q.question_type === 'multiple_choice' ? t('teacher.editAssignment.typeMultipleChoice') : t('teacher.editAssignment.typeTrueFalse')}
                      </span>
                      {topic.name && (
            ...
            <p className="text-sm text-gray-500">{t('teacher.editAssignment.noQuestions')}</p>
          )}
        </div>
      )}
    </div>
