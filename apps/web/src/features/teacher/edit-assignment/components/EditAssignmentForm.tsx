...
export const EditAssignmentForm: React.FC<EditAssignmentFormProps> = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm">
      <div>
        <Label required>{t('teacher.editAssignment.titleLabel')}</Label>
        <Input
          required
          type="text"
          value={form.title}
      ...
      <div>
        <Label>{t('teacher.editAssignment.descLabel')}</Label>
        <Textarea
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          <Label>{t('teacher.editAssignment.modeLabel')}</Label>
          <Select
            value={form.mode}
            onChange={(value) => setForm({ ...form, mode: value })}
            options={[
              { value: 'adaptive', label: t('teacher.editAssignment.modeAdaptive') },
              { value: 'standard', label: t('teacher.editAssignment.modeStandard') },
              { value: 'exam', label: t('teacher.editAssignment.modeExam') }
            ]}
          />
        </div>

        <div>
          <Label>{t('teacher.editAssignment.maxAttemptsLabel')}</Label>
          <Input
            type="number"
            min="0"
          ...
          />
          <p className="text-xs text-slate-500 font-medium mt-1">{t('teacher.editAssignment.noLimitHint')}</p>
        </div>

        <div>
          <Label>{t('teacher.editAssignment.timeLimitLabel')}</Label>
          <Input
            type="number"
            min="0"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <Label>{t('teacher.editAssignment.deadlineLabel')}</Label>
          <Input
            type="datetime-local"
            value={form.deadline}
        ...
        <div>
          <Label>{t('teacher.editAssignment.assignToLabel')}</Label>
          <Select
            ...
            options={[
              { value: 'all', label: t('teacher.editAssignment.assignAll') },
              { value: 'specific', label: t('teacher.editAssignment.assignSpecific') }
            ]}
          />
        </div>
      </div>
      ...
      {!form.assignToAll && (
        <div className="bg-indigo-50/50 p-5 rounded-2xl border border-indigo-100">
          ...
          <div className="bg-white rounded-xl border border-slate-200 max-h-[300px] overflow-y-auto space-y-1 p-2">
            {members.length === 0 ? (
              <p className="text-sm text-slate-500 text-center p-4 font-medium">{t('teacher.editAssignment.noStudents')}</p>
            ) : (
              members.map((member) => (
      ...
      <div>
        <Label className="mb-3">{t('teacher.editAssignment.selectTopicsLabel')}</Label>
