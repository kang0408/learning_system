...
export const KnowledgeRadarChart: React.FC<KnowledgeRadarChartProps> = ({ topicPerformance }) => {
  ...
  if (!topicPerformance || topicPerformance.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm h-full">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Biểu đồ Năng lực (Knowledge Graph)</h3>
        <p className="text-sm text-gray-500 text-center py-10">Chưa có đủ dữ liệu để phân tích.</p>
      </div>
    );
  }
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context: any) {
            return `Độ chính xác: ${context.raw}%`;
          }
        }
      }
    },
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-gray-900">Biểu đồ Năng lực</h3>
        <p className="text-sm text-gray-500 mt-1">Mức độ thông thạo theo từng lĩnh vực gốc</p>
      </div>
        ...
        {labels.length >= 3 ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-gray-500 text-sm p-6 bg-gray-50 rounded-xl border border-dashed border-gray-200">
              Cần ít nhất 3 chủ đề gốc để hiển thị biểu đồ mạng nhện. Hiện tại học sinh mới học {labels.length} chủ đề.
            </div>
          </div>
        )}
      </div>
